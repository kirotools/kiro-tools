...
impl RateLimitTracker {
    ...
    pub fn parse_from_error(
        ...
        let reason = if status == 429 {
            tracing::warn!("429 Error Body: {}", body);
            self.parse_rate_limit_reason(body)
        } else if status == 404 {
            tracing::warn!("404: model unavailable on this account, short lockout before rotation");
    ...
    fn parse_retry_time_from_body(&self, body: &str) -> Option<u64> {
        ...
        if trimmed.starts_with('{') || trimmed.starts_with('[') {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(trimmed) {
                // 1. 常见的 quotaResetDelay 格式 (支持所有格式："2h1m1s", "1h30m", "42s", "500ms" 等)
