import { ArrowRightLeft, RefreshCw, Trash2, Download, Info, Lock, Ban, Diamond, Gem, Circle, ToggleLeft, ToggleRight } from 'lucide-react';
        ...
        <tr className={cn(
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        {account.quota?.subscription_tier && (() => {
                            ...
                            } else if (tier.includes('pro') && (tier.includes('pro+') || tier.includes('pro_plus') || tier.includes('proplus'))) {
                                return (
                                    <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-gradient-to-r from-violet-600 to-purple-600 text-white text-[10px] font-bold shadow-sm hover:scale-105 transition-transform cursor-default">
                                        <Gem className="w-2.5 h-2.5 fill-current" />
                                        PRO+
                                    </span>
                                );
            ...
            <td className="px-4 py-1">
                {account.quota?.is_forbidden ? (
                    ...
                    <div className="grid grid-cols-1 gap-x-4 gap-y-1 py-0">
                        <div className="relative h-[22px] flex items-center px-1.5 rounded-md overflow-hidden border border-gray-100/50 dark:border-white/5 bg-gray-50/30 dark:bg-white/5 group/quota">
                            {creditModel && (
                                <div
                                    className={`absolute inset-y-0 left-0 transition-all duration-700 ease-out opacity-15 dark:opacity-20 ${getColorClass(creditModel.percentage)}`}
                            ...
                            <div className="relative z-10 w-full flex items-center text-[10px] font-mono leading-none">
                                ...
                                <span className={cn("w-[36px] text-right font-bold transition-colors",
                                    getQuotaColor(creditModel?.percentage || 0) === 'success' ? 'text-emerald-600 dark:text-emerald-400' :
                                        getQuotaColor(creditModel?.percentage || 0) === 'warning' ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'
                                )}>
                                    {creditModel
