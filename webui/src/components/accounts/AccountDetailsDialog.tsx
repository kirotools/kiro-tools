                ...
                <div className="px-6 py-5 border-b border-gray-100 dark:border-base-200 bg-gray-50/50 dark:bg-base-200/50 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        {account.quota?.subscription_tier && (() => {
                            ...
                            const tierClass = tier.includes('power') ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                                (tier.includes('pro+') || tier.includes('pro_plus') || tier.includes('proplus')) ? 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400' :
                                tier.includes('pro') ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                                'bg-gray-100 text-gray-600 dark:bg-base-300 dark:text-gray-400';
                            // Extract display name from Q_DEVELOPER_STANDALONE_POWER → POWER
                            const displayTier = tier.includes('power') ? 'POWER' :
                                (tier.includes('pro+') || tier.includes('pro_plus') || tier.includes('proplus')) ? 'PRO+' :
                                tier.includes('pro') ? 'PRO' : 'FREE';
                            return (
                                <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${tierClass}`}>
                                    {displayTier}
                                </div>
                            );
                        })()}
