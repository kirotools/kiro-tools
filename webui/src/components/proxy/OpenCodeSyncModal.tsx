...
interface OpenCodeSyncModalProps {
    proxyUrl: string;
    apiKey: string;
    getFormattedProxyUrl: (app: 'Claude' | 'Codex' | 'OpenCode' | 'Droid') => string;
    onClose: () => void;
    onSyncDone: () => void;
}
...
export function OpenCodeSyncModal({ proxyUrl, apiKey, onClose, onSyncDone }: OpenCodeSyncModalProps) {
    ...
    const { models: kiroModels } = useProxyModels();
    ...
    const rebuildPreview = useCallback((selectedIds: Set<string>) => {
        const selected = kiroModels.filter(m => selectedIds.has(m.id));
    ...
    const allSelected = kiroModels.length > 0 && kiroModels.every(m => selectedModels.has(m.id));
    const toggleAll = () => {
        const next = allSelected ? new Set<string>() : new Set(kiroModels.map(m => m.id));
    ...
    const groups = [...new Set(kiroModels.map(m => m.group))];
                ...
                <div className="px-5 pb-3 shrink-0 border-b border-gray-100 dark:border-base-200">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                            ...
                            <span className="ml-2 text-gray-300">{selectedModels.size}/{kiroModels.length}</span>
                        </span>
                    ...
                    <div className="space-y-2 max-h-[25vh] overflow-auto">
                        {groups.map(group => {
                            const groupModels = kiroModels.filter(m => m.group === group);
                            return (
                                <div key={group}>
