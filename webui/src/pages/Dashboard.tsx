import { AlertTriangle, ArrowRight, Bot, Download, RefreshCw, Users } from 'lucide-react';
...
function Dashboard() {
    ...
    const stats = useMemo(() => {
        const creditQuotas = accounts
            .map(a => a.quota?.models.find(m => m.name.toLowerCase() === 'kiro-credit')?.percentage || 0)
            .filter(q => q > 0);

        const lowQuotaCount = accounts.filter(a => {
            if (a.quota?.is_forbidden) return false;
            const credit = a.quota?.models.find(m => m.name.toLowerCase() === 'kiro-credit')?.percentage || 0;
            return credit < 20;
        }).length;

        return {
            total: accounts.length,
            avgCredit: creditQuotas.length > 0
                ? Math.round(creditQuotas.reduce((a, b) => a + b, 0) / creditQuotas.length)
                : 0,
            ...
            >
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    ...
                    <div className="bg-white dark:bg-base-100 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-base-200">
                        ...
                        <div className="text-2xl font-bold text-gray-900 dark:text-base-content mb-0.5">{stats.avgCredit}%</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">{t('dashboard.avg_claude')}</div>
                        {stats.avgCredit > 0 && (
                            <div className={`text-[10px] mt-1 ${stats.avgCredit >= 50 ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}>
                                {stats.avgCredit >= 50 ? t('dashboard.quota_sufficient') : t('dashboard.quota_low')}
                            </div>
                        )}
                    </div>
