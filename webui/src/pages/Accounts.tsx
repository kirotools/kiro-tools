  ...
  const filterCounts = useMemo(() => {
    return {
        ...
        pro_plus: searchedAccounts.filter((a) => {
            ...
            return !tier.includes("power") && (tier.includes("pro+") || tier.includes("pro_plus") || tier.includes("proplus"));
        }).length,
        pro: searchedAccounts.filter((a) => {
            const tier = a.quota?.subscription_tier?.toLowerCase() || '';
            return tier.includes("pro") && !tier.includes("power") && !tier.includes("pro+") && !tier.includes("pro_plus") && !tier.includes("proplus");
        }).length,
      ...
      <div className="flex-none flex items-center gap-2">
        ...
        <div className="flex gap-0.5 bg-gray-100/80 dark:bg-base-200 p-1 rounded-xl border border-gray-200/50 dark:border-white/5 shrink-0">
          ...
          <button
            ...
            title={`${t('accounts.power')} (${filterCounts.power})`}
          >
            <span className="hidden md:inline">{t('accounts.power')}</span>
            <span className={cn(
              "px-1.5 py-0.5 rounded-md text-[10px] font-bold transition-colors",
              filter === 'power'
                ? "bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400"
          ...
          <button
            ...
            onClick={() => setFilter('pro_plus')}
            title={`PRO+ (${filterCounts.pro_plus})`}
          >
