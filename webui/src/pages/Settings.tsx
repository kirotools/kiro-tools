...
import { Save, Github, User, ExternalLink, RefreshCw, LayoutDashboard, Users, Network, Activity, BarChart3, Settings as SettingsIcon, Lock, CheckCircle2, Globe } from 'lucide-react';
...
function Settings() {
    ...
    const [dataDirPath, setDataDirPath] = useState<string>('~/.kiro_tools/');

    // Kiro cache clearing state
    ...
    const handleOpenClearCacheDialog = async () => {
        try {
            const paths = await invoke<string[]>('get_kiro_cache_paths');
            setCachePaths(paths);
    ...
    const confirmClearKiroCache = async () => {
    ...
    return (
        <div className="h-full w-full overflow-y-auto">
            <div className="p-5 space-y-4 max-w-7xl mx-auto">
                    ...
                    {activeTab === 'advanced' && (
                        <>
                            <div className="space-y-4">
                                ...
                                {/* Kiro 缓存清理 */}
                                <div className="border-t border-gray-200 dark:border-base-200 pt-4">
                                    <h3 className="font-medium text-gray-900 dark:text-base-content mb-3">{t('settings.advanced.kiro_cache_title', 'Kiro Tools 缓存清理')}</h3>
                                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-3 mb-3">
                                        <p className="text-sm text-amber-700 dark:text-amber-400">{t('settings.advanced.kiro_cache_warning', '请确保 Kiro Tools 应用已完全退出后再执行清理操作。')}</p>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-base-200 border border-gray-200 dark:border-base-300 rounded-lg p-3 mb-3">
                                        <p className="text-sm text-gray-600 dark:text-gray-400">{t('settings.advanced.kiro_cache_desc', '清理 Kiro Tools 应用的缓存可以解决登录失败、版本验证错误、OAuth 授权失败等问题。')}</p>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <button
                                        ...
                                        >
                                            {t('settings.advanced.clear_kiro_cache', '清理 Kiro Tools 缓存')}
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-3xl px-4">
                                    ...
                                    <div className="bg-white dark:bg-base-100 p-4 rounded-2xl border border-gray-100 dark:border-base-300 shadow-sm hover:shadow-md hover:border-blue-200 dark:hover:border-blue-800 transition-all group flex flex-col items-center text-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="font-bold text-gray-900 dark:text-base-content">kirotools</div>
                                        </div>
                                    </div>

                                    {/* GitHub Card */}
                                    <a
                                        href="https://github.com/kirotools/kiro-tools"
                                        target="_blank"
                                        rel="noreferrer"
                ...
                {/* Kiro Cache Clear Modal */}
                <ModalDialog
                    isOpen={isClearCacheOpen}
                    title={t('settings.advanced.clear_cache_confirm_title', '确认清理 Kiro Tools 缓存')}
                    type="confirm"
                    confirmText={isClearingCache ? t('common.clearing', '清理中...') : t('common.clear')}
                    cancelText={t('common.cancel')}
                    isDestructive={true}
                    onConfirm={confirmClearKiroCache}
                        ...
                        {cachePaths.length > 0 ? (
                            ...
                            <div className="bg-gray-50 dark:bg-base-200 rounded-lg p-3">
                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                    {t('settings.advanced.cache_not_found', '未找到 Kiro Tools 缓存目录')}
                                </p>
                            </div>
                        )}
                        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-2">
                            <p className="text-xs text-amber-700 dark:text-amber-400">
                                {t('settings.advanced.kiro_cache_warning', '请确保 Kiro Tools 应用已完全退出后再执行清理操作。')}
                            </p>
                        </div>
                    </div>
                </ModalDialog>
